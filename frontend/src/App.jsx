...
function App() {
  ...
  const [finalpasswort, setFinalpasswort] = useState("");
  const [finalbenutzername, setFinalbenutzername] = useState("");
          ...
          categoriesData.map(async (category) => {
            try {
              const resCounts = await fetch(
                `http://localhost:3050/category_task_counts/${category.id}`,
              );
              ...
              console.error(
                `Fehler beim Laden der Counts für Kategorie ${category.id}:`,
                countError,
              );
              // Fallback, wenn Counts nicht geladen werden können
              return {
                ...category,
                counts: { open_tasks: 0, completed_tasks: 0 },
              };
            }
          }),
        );
              ...
              ? // Erhöhe open_tasks um 1, stelle sicher, dass Counts existiert
                {
                  ...
                  counts: {
                    ...cat.counts,
                    open_tasks: (cat.counts?.open_tasks ?? 0) + 1,
                  },
                }
              : cat,
          ),
  ...
  const categoryLoeschen = async (id) => {
    ...
    if (
      ...
      const confirmDelete = window.confirm(
        `Die Kategorie "${categoryToDelete.name}" enthält noch Aufgaben. Wirklich löschen? Alle zugehörigen Aufgaben gehen verloren!`,
      );
      if (!confirmDelete) {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:3050/delete_category/${id}`,
        { method: "DELETE" },
      );
      ...
      if (!response.ok) {
        console.error(
          "Fehler beim Löschen der Kategorie auf dem Server. Status:",
          response.status,
        );
        // Rollback: Füge die Kategorie wieder zum State hinzu
        if (categoryToDelete) {
          // Nur wenn wir die Kategorie vorher gefunden haben
          setCategories((prevCategories) =>
            [...prevCategories, categoryToDelete].sort((a, b) => a.id - b.id),
          );
        }
      ...
      console.error(
        "Fehler beim Löschen einer Kategorie (Fetch oder Server):",
        error,
      );
  ...
  const taskStatusAktualisieren = (id, completed) => {
        ...
        if (cat.id === selectedCategory) {
          ...
          return {
            ...
            counts: {
              ...
              completed_tasks:
                (cat.counts?.completed_tasks ?? 0) + completedAdjustment,
            },
          };
        }
        return cat;
    ...
    fetch(`http://localhost:3050/update_completed/${id}`, {
      ...
      .then((response) => {
        ...
        if (!response.ok) {
          console.error(
            "Fehler beim Aktualisieren des Status auf dem Server. Status:",
            response.status,
          );
      ...
      .catch((error) => {
        console.error(
          "Fehler beim Aktualisieren des Status (Fetch oder Server):",
          error,
        );
  ...
  const itemLoeschen = (id) => {
    ...
    fetch(`http://localhost:3050/delete/${id}`, {
      ...
      .then((response) => {
        ...
        if (!response.ok) {
          console.error(
            "Fehler beim Löschen des Tasks auf dem Server. Status:",
            response.status,
          );
          // Rollback Task: Füge ihn wieder hinzu
          setTasks((prevTasks) =>
            [...prevTasks, taskToDelete].sort((a, b) => a.id - b.id),
          ); // Sortierung optional
          // Rollback Counts
          setCategories((prevCategories) =>
            prevCategories.map((cat) => {
      ...
      .catch((error) => {
        console.error(
          "Fehler beim Löschen einer Aufgabe (Fetch oder Server):",
          error,
        );
  ...
  const notizAktualisieren = (id, note) => {
    ...
    setTasks((prevTasks) =>
      prevTasks.map((task) => (task.id === id ? { ...task, note } : task)),
    );
    ...
    fetch(`http://localhost:3050/update_note/${id}`, {
      ...
      .then((response) => {
        ...
        if (!response.ok) {
          console.error(
            "Fehler beim Aktualisieren der Notiz auf dem Server. Status:",
            response.status,
          );
          // Rollback Note
          setTasks((prevTasks) =>
      ...
      .catch((error) => {
        console.error(
          "Fehler beim Aktualisieren der Notiz (Fetch oder Server):",
          error,
        );
        // Rollback ist oben passiert oder Fetch fehlgeschlagen
        // Optional: Fehlermeldung
...
const handleAnmeldung= () => {
    setFinalbenutzername(benutzername);
    setFinalpasswort(passwort);
    setLoggedin(true);
}
...
const handleRegistrierung= () => {
  ...
  return (
    ...
    <div
      className={`todo-app container ${darkMode ? "dark" : ""}`}
    ...
    >
      ...
      {loggedin ? (      <div className="category-selection">
        {!selectedCategory ? (
          <>
            ...
            <div className="category-buttons">
              {" "}
              {/* Klassennamen können natürlich angepasst werden */}
              {categories.map(({ id, name, counts }) => (
                <div key={id} className="category-item">
                  ...
                  >
                    ...
                    {name}{" "}
                    {counts
                      ? `(${counts.open_tasks ?? 0}/${counts.completed_tasks ?? 0})`
                      : ""}
                  </button>
                  {/* Löschbutton */}
                  <button
                    onClick={() => categoryLoeschen(id)}
      ...
      </div>) : (<Login handleBenutzername={handleBenutzername} handlePasswort={handlePasswort} handleAnmeldung={handleAnmeldung} handleRegistrierung={handleRegistrierung}/>)}
        ...
        <>
          ...
          <ul className="task-list">
            ...
            {tasks
              ...
              .map(({ id, title, completed, deadline, note }) => (
                <li key={id}>
                  {/* Checkbox */}
                  <input
                    type="checkbox"
                  ...
                  <p>
                    Deadline:{" "}
                    {deadline ? new Date(deadline).toLocaleString() : "Keine"}
                  </p>
